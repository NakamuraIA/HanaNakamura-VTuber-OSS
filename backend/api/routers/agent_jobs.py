...
from backend.api.services.agent_jobs import AgentJobManager


router = APIRouter(tags=["Agente — tarefas"])
...
@router.get("/api/agent-jobs", summary="Listar tarefas em segundo plano")
...
@router.get("/api/agent-jobs/{job_id}", summary="Ver uma tarefa")
...
@router.post("/api/agent-jobs/{job_id}/cancel", summary="Cancelar uma tarefa")
...
@router.post("/api/agent-jobs/cancel-active", summary="Cancelar tudo que está rodando")
