...
from backend.api.services.catalog import DEFAULT_CHAT_CONFIG, DEFAULT_CONNECTIONS, DEFAULT_LLM_CONFIG
from backend.api.services.chat import run_text_turn
...
@router.get("/outbox", summary="Mensagens que a Hana quer te mandar por DM")
async def discord_outbox(request: Request) -> dict[str, Any]:
    """Pending DMs Hana queued for Nakamura; the Discord bot polls this."""
    from backend.tools.discord_tools import pending_outbox
    from backend.discord_bot.owner import primary_owner_id

    return {"ok": True, "ownerId": primary_owner_id(), "pending": pending_outbox(request.app.state.memory)}


@router.post("/outbox/delivered", summary="Marcar DMs como entregues")
async def discord_outbox_delivered(request: Request, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    from backend.tools.discord_tools import mark_delivered
...
@router.post("/message", summary="Mandar uma mensagem do Discord pra Hana")
