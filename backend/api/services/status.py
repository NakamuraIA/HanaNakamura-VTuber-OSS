...
from backend.api.services.catalog import DEFAULT_CHAT_CONFIG, DEFAULT_CONNECTIONS, DEFAULT_LLM_CONFIG, DEFAULT_VOICE_CONFIG
...
def logs_payload(memory: MemoryStore, *, limit: int = 100) -> dict[str, Any]:
    """Ultimas linhas do log, lidas do BANCO (tabela chat_log).

    Antes vinha do hana_events.jsonl. O problema nao era o formato: e que
    `recent_events` le o ARQUIVO INTEIRO e joga fora tudo menos as ultimas N —
    com 5 MB de historico, abrir a aba Logs relia 5 MB. Aqui o LIMIT roda no
    SQLite.

    De quebra o log melhora: `chat_log` guarda o canal e o meta da ferramenta,
    entao da pra ver de ONDE veio cada linha e o que falhou.

    Cai de volta no jsonl se a tabela estiver vazia (banco recem-migrado).
    """
    itens = _logs_do_banco(memory, limit)
    if itens:
        return {"logs": itens}
    return {
        "logs": [
            ...
            for item in memory.recent_events(limit=limit)
        ]
    }


def _logs_do_banco(memory: MemoryStore, limit: int) -> list[dict[str, Any]]:
    """Le do chat_log. Devolve [] em qualquer falha — log nunca derruba a tela."""
    try:
        from backend.memory.core import HanaMemory

        with HanaMemory(str(memory.db_path))._connect() as conn:
            linhas = conn.execute(
                "SELECT created_at, role, author, content, channel, meta_json"
    ...
    for r in reversed(linhas):  # mais antigo primeiro, como a tela espera
        meta = r["meta_json"] or ""
        # Turno que falhou e a linha que a Nakamura procura quando algo deu
        # errado; marcar como ERROR faz ela saltar aos olhos no meio do resto.
        nivel = "ERROR" if '"error"' in meta and '"error": null' not in meta else "INFO"
        saida.append(
            {
                "timestamp": r["created_at"],
                "level": nivel,
                "logger": r["channel"] or "backend",
                "message": f"{r['author'] or r['role']}: {r['content']}",
            }
        )
    return saida
...
def status_payload(memory: MemoryStore | None = None) -> dict[str, Any]:
    ...
    tts_provider = configured_tts_provider or tts_registry.DEFAULT_TTS_PROVIDER
