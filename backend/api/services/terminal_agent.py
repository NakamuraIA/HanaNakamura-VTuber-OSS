...
from backend.modules.voice.tts_readable import sanitize_tts_text, tts_payload
...
_stream_subscribers: set[tuple[asyncio.AbstractEventLoop, asyncio.Queue[dict[str, Any]]]] = set()
_stream_subscribers_lock = threading.Lock()


def subscribe_terminal_stream() -> tuple[asyncio.AbstractEventLoop, asyncio.Queue[dict[str, Any]]]:
    """Assina deltas efêmeros do Terminal sem gravá-los na memória."""
    subscription = (asyncio.get_running_loop(), asyncio.Queue())
    with _stream_subscribers_lock:
        _stream_subscribers.add(subscription)
    return subscription


def unsubscribe_terminal_stream(
    subscription: tuple[asyncio.AbstractEventLoop, asyncio.Queue[dict[str, Any]]],
) -> None:
    with _stream_subscribers_lock:
        _stream_subscribers.discard(subscription)


def publish_terminal_stream(event: dict[str, Any]) -> None:
    """Entrega um delta ao vivo com segurança mesmo vindo da thread do microfone."""
    with _stream_subscribers_lock:
        subscribers = tuple(_stream_subscribers)
...
def append_terminal_event(memory: MemoryStore, payload: dict[str, Any]) -> dict[str, Any]:
    """Registra uma projeção visual, nunca uma nova fala da conversa.

    A fala canônica já é salva por ``run_text_turn`` no canal de origem. Esta
    marca mantém o evento disponível no Terminal sem duplicá-lo na memória.
    """
