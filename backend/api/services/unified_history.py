...
def context_size_report(
    ...
    *,
    ...
    "where do the 17k go" into a number Nakamura sees every turn.
    """
    blocks: dict[str, int] = {}
...
def _is_attachment_record(item: dict[str, Any]) -> bool:
    """True para o registro-indice de anexo (nao e lembranca de verdade).

    Checa kind/source no topo E no metadata porque linhas antigas (pre-migracao
    v2) guardavam so no metadata.
    """
    metadata = item.get("metadata") if isinstance(item.get("metadata"), dict) else {}
    kind = str(item.get("kind") or metadata.get("kind") or "")
    source = str(item.get("source") or metadata.get("source") or "")
    return kind == "attachment" or source == "chat_attachment"
...
def select_memories_for_context(
    ...
    *,
    ...
    Priority order (deduplicated by id): pinned (always) -> search hits on the
    current query (ranked) -> most-recent active memories.

    Nota: o passo 2 usa MemoryStore.search, que combina FTS/BM25 COM similaridade
    vetorial quando a camada semantica esta ligada. Ou seja, existe SIM calculo de
    embedding da query no caminho da requisicao — o comentario antigo aqui dizia
    "100% SQLite/FTS, zero latencia" e ficou desatualizado quando a semantica
    entrou.

    Registro de ANEXO nao entra no contexto: e um indice de arquivo
    ("Anexo salvo para consulta futura: foto.png (image/png, 258044 bytes)"), nao
    uma lembranca. Como nasce a cada imagem colada, era sempre o mais recente e
    dominava o passo 3, gastando milhares de tokens por turno com texto que nao
    ajuda o modelo. O lookup de anexo continua intacto: AttachmentStore.recent()
    le direto de list_memories().
    """
    ...
    try:
        ...
        _extend(memory.list_memories(limit=50, status="pinned", exclude_attachments=True))
        # 2) Query-relevant hits (FTS/BM25 + vetorial, quando a semantica esta on).
        clean_query = str(query or "").strip()
        if clean_query:
            # touch=False: quem conta o uso e o build_memory_context_block, depois
            # de saber o que REALMENTE entrou no prompt. Contar aqui premiava
            # tambem o que a busca trouxe e o corte de orcamento descartou.
            _extend(memory.search(clean_query, limit=max_items, touch=False))
        # 3) Backfill with the freshest active memories so she is never "blank".
        #    exclude_attachments no SQL: filtrar depois do LIMIT devolvia lista
        #    vazia quando os anexos (sempre os mais recentes) ocupavam o limite.
        _extend(memory.list_memories(limit=max_items, status="active", exclude_attachments=True))
    except Exception:
...
def build_recent_activity_block(memory: MemoryStore) -> str:
    ...
    deliberately generous with tokens, because Nakamura prefers spending tokens
    ...
    return (
        ...
        "Use-o para retomar trabalhos de onde parou: se a Nakamura pedir para 'continuar' algo, "
...
def build_latest_diary_block(memory: MemoryStore) -> tuple[str, dict[str, Any] | None]:
    ...
    """
    try:
        from backend.memory.long_term.sleep import latest_episode
...
def build_memory_context_block(
    ...
    *,
    ...
    # Uso e contado AQUI, so no que entrou no prompt de verdade — nao no que a
    # busca levantou. Antes o `search` marcava tudo que rankeava, entao 13
    # memorias chegaram a 386 "usos" e ficaram travadas no topo pra sempre,
    # sufocando qualquer fato novo.
    if rag_items:
        try:
            memory.mark_memories_accessed([str(item.get("id")) for item in rag_items])
        except Exception:
            logger.debug("Falha ao marcar memorias como usadas", exc_info=True)
...
CHANNEL_VOICE = "voice"
ALL_CHANNELS = (CHANNEL_CONTROL_CENTER, CHANNEL_TERMINAL_AGENT, CHANNEL_DISCORD, CHANNEL_VOICE)

# Quem enxerga o historico de quem.
#
# BUG que isto conserta: `build_unified_history` recebia `channel`, mas so usava
# pra escolher o LIMITE — o filtro era `if event_channel not in ALL_CHANNELS`,
# uma tautologia que nao filtrava nada. Resultado: uma resposta no Discord vinha
# com a conversa de voz de uma hora antes colada no contexto.
#
# Voz e terminal ficam JUNTOS de proposito: sao o mesmo lugar fisico (voce na
# frente do PC) e a voz ja grava seus eventos como `terminal_agent`. Separar os
# dois cortaria a conversa no meio quando voce alterna entre falar e digitar.
CHANNEL_GROUPS: dict[str, frozenset[str]] = {
    CHANNEL_CONTROL_CENTER: frozenset({CHANNEL_CONTROL_CENTER, "chat"}),
    CHANNEL_DISCORD: frozenset({CHANNEL_DISCORD}),
    CHANNEL_TERMINAL_AGENT: frozenset({CHANNEL_TERMINAL_AGENT, CHANNEL_VOICE}),
    CHANNEL_VOICE: frozenset({CHANNEL_TERMINAL_AGENT, CHANNEL_VOICE}),
}


def channels_visible_to(channel: str) -> frozenset[str]:
    """Canais cujo historico o canal pedinte pode ver.

    Canal desconhecido cai no grupo do control center em vez de ver tudo: se um
    canal novo aparecer sem ser mapeado aqui, ele fica isolado por padrao — o
    erro seguro e ver de menos, nunca vazar de mais.
    """
    return CHANNEL_GROUPS.get(str(channel or "").strip().lower(), CHANNEL_GROUPS[CHANNEL_CONTROL_CENTER])
...
_VOICE_STYLE_HINT = (
    ...
    "- Quando fizer sentido, dirija-se diretamente à Nakamura pelo nome.\n"
    "- Nao finalize toda fala com pergunta de suporte. Reaja, provoque de leve ou dê o próximo passo em 1 frase.\n"
    "- Se Nakamura reclamar que voce parece robotica, mude o tom imediatamente; nao responda com promessa burocratica.\n"
...
_DISCORD_TEXT_STYLE_HINT = (
    ...
    "Você está conversando por TEXTO no Discord, em um chat PRIVADO só seu com a Nakamura "
    "(o bot é privado dela). Trate sempre quem fala como a Nakamura.\n"
...
_CALL_MODE_HINT = (
    ...
    "chega NÃO é necessariamente da Nakamura — pode ser qualquer participante.\n"
    "REGRAS OBRIGATÓRIAS:\n"
    "- NÃO assuma que quem falou é a Nakamura. Só trate como ela se o conteúdo deixar claro "
    ...
    "reaja ao papo coletivo. Não direcione tudo à Nakamura nem fique pedindo comando a ela.\n"
    ...
    "- Não invente que a Nakamura disse/fez algo que veio de outra pessoa.\n"
...
def build_unified_history(
    ...
    *,
    ...
    visiveis = channels_visible_to(channel)
    filtered: list[dict[str, Any]] = []
    for event in raw_events:
        event_channel = str(event.get("channel") or "").strip().lower()
        if event_channel not in visiveis:
            continue
        role = event.get("role", "")
        ...
        if not content:
            continue
        metadata = event.get("metadata") if isinstance(event.get("metadata"), dict) else {}
        if metadata.get("conversation") is False:
            continue
