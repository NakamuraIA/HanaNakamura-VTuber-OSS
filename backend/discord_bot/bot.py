...
from backend.discord_bot.backend_client import HanaBackendClient
from backend.discord_bot.owner import is_owner

logger = logging.getLogger(__name__)

# Janela para descartar re-entrega do MESMO evento de mensagem (message.id).
# O Discord pode reenviar um message_create depois de um reconnect/resume, e
# o discord.py dispara on_message de novo sem dedup — o que fazia a Hana
# responder 2-3x a mesma mensagem e ainda gravar o texto 3x no histórico.
# Como cada mensagem tem um id único, guardar ids por alguns segundos nunca
# engole duas mensagens diferentes, só reenvios do mesmo evento.
...
class OwnerOnlyTree(app_commands.CommandTree):
    ...
    A Hana é privada: só a Nakamura pode usar. Outros recebem uma resposta
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        ...
        try:
            await interaction.response.send_message(
                "🔒 Esse bot é privado da Nakamura. Você não tem acesso.",
                ephemeral=True,
            )
        except Exception:
            pass
...
class HanaBot(commands.Bot):
    ...
    def __init__(self) -> None:
        ...
        super().__init__(
            ...
            description="Hana AI - chatbot privado da Nakamura",
        )
        self.backend = HanaBackendClient()
        self._recent_message_at: dict[int, float] = {}

    async def setup_hook(self) -> None:
        """Carrega cogs e sincroniza os slash commands globalmente (user-install)."""
        for cog in ("geral", "hana", "voz", "config"):
            try:
                await self.load_extension(f"backend.discord_bot.cogs.{cog}")
    ...
    async def _outbox_poller(self) -> None:
        """Deliver DMs Hana queued in the backend outbox, mentioning Nakamura."""
        try:
    ...
    def _is_duplicate_message(self, message: discord.Message) -> bool:
        """True se este evento de mensagem já foi processado há pouco.

        O Discord às vezes reentrega o mesmo ``message_create`` (reconnect/resume),
        o que faria ``on_message`` rodar 2-3x com o mesmo ``message.id`` e a Hana
        responder lá e no histórico várias vezes. Como o id é único por mensagem,
        descartar por janela de tempo não engole duas mensagens distintas.
        """
        now = time.monotonic()
        # poda otimista para não crescer pra sempre em chats movimentados
        for msg_id in list(self._recent_message_at):
            if now - self._recent_message_at[msg_id] > _DEDUP_WINDOW_SECONDS:
                del self._recent_message_at[msg_id]
        if message.id in self._recent_message_at:
            return True
        self._recent_message_at[message.id] = now
        return False
    ...
    async def on_message(self, message: discord.Message) -> None:
        ...
        if self._is_duplicate_message(message):
            return  # re-entrega do Discord do mesmo evento: já respondemos uma vez
