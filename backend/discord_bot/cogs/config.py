...
Categorias: Chat (o que o Discord usa), Agente (loop de ferramentas),
Multimodal (fallback de imagem/audio/video) e Imagem.
...
CATEGORY_META = {
    ...
    "multimodal": {"label": "Multimodal (Fallback)", "emoji": "👁️"},
...
def provider_ids_for(category: str, catalog: dict[str, Any]) -> list[str]:
    ...
    if category == "multimodal":
        providers = {
            ...
            if isinstance(item, dict) and item.get("supportsVision") and item.get("provider")
        }
        return [str(provider) for provider in (catalog.get("llmProviders") or []) if str(provider) in providers]
...
def model_ids_for(category: str, provider: str, catalog: dict[str, Any]) -> list[dict[str, str]]:
    ...
    for item in source:
        if not isinstance(item, dict) or item.get("provider") != provider:
            continue
        # Domínio explícito: só conversa entra em chat/agente/multimodal.
        # (O /api/catalog já filtra; aqui é defesa em profundidade caso o
        # catálogo venha de outra fonte ou ganhe linhas sem filtro.)
        if category not in {"imagem"} and str(item.get("modelDomain") or "chat") != "chat":
            continue
        if category == "multimodal" and not item.get("supportsVision"):
            continue
        if category not in {"imagem", "multimodal"} and "text" not in (item.get("outputModalities") or ["text"]):
            continue
...
def build_config_calls(category: str, provider: str, model: str) -> list[tuple[str, dict[str, Any]]]:
    ...
    - Imagem usa o campo generico imageModel quando o provider permite escolha.
    """
    ...
    if category == "multimodal":
        return [("update_llm_config", {"visionProvider": p, "visionModel": m})]
    if category == "imagem":
        payload: dict[str, Any] = {"imageProvider": p}
        if m:
            payload["imageModel"] = m
...
class ConfigCog(commands.Cog):
    ...
    async def status(self, interaction: discord.Interaction) -> None:
        ...
        lines = [
            ...
            + (f" / `{image.get('imageModel') or image.get('openrouterImageModel')}`" if (image.get("imageModel") or image.get("openrouterImageModel")) else ""),
            f"👁️ **Multimodal:** {_provider_label(str(llm.get('visionProvider') or ''))} / `{llm.get('visionModel') or '?'}`",
        ]
