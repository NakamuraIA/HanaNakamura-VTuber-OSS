...
from backend.mcp.client import McpEnvMissing, McpSdkUnavailable, McpStdioClient
from backend.mcp.config import McpConfigStore
from backend.mcp.contracts import McpCallRequest, McpServerConfig, McpToolInfo
...
class McpManager:
    ...
    def start_enabled_background(self) -> None:
        """Aquece os MCPs ativos sem bloquear a inicialização da Hana."""
        for server in self.config_store.list_servers():
            if server.enabled:
                self.client.warmup_background(server)
            else:
                self.client.mark_disabled(server.id)
    ...
    def enable_server(self, server_id: str, enabled: bool) -> dict[str, Any]:
        ...
        if enabled:
            ...
            self.client.close_server_background(server.id)
    ...
    def update_server_config(self, server_id: str, updates: dict[str, Any]) -> dict[str, Any]:
        """Update command, args, env, cwd, or timeout of an MCP server."""
        allowed_fields = {"command", "args", "env", "cwd", "timeout", "name"}
        filtered = {key: value for key, value in updates.items() if key in allowed_fields}
        if not filtered:
            return {"status": "error", "error": "no_valid_fields"}
        previous = self.config_store.get_server(server_id)
        server = self.config_store.update_server(server_id, **filtered)
        if not server:
            return {"status": "error", "error": "mcp_server_not_found", "server_id": server_id}
        process_changed = previous is None or self._process_signature(previous) != self._process_signature(server)
        if server.enabled and process_changed:
            self.client.restart_background(server)
        elif server.enabled:
            self.client.warmup_background(server)
        else:
            self.client.close_server_background(server.id)
    ...
    def upsert_preset(self, preset_id: str) -> dict[str, Any]:
        ...
        previous = self.config_store.get_server(preset_id)
    ...
    @staticmethod
    def _process_signature(server: McpServerConfig) -> tuple[Any, ...]:
        return (
            server.command,
            tuple(server.args),
            tuple(sorted((str(key), str(value)) for key, value in server.env.items())),
            server.cwd,
        )
    ...
    def _server_payload(self, server: McpServerConfig) -> dict[str, Any]:
        ...
        runtime = self.client.runtime_status(server.id) if server.enabled else {"status": "disabled"}
        data["runtime_status"] = runtime.get("status") or "warming"
        return data
