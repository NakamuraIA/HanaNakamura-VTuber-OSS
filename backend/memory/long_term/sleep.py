...
SUMMARY_PROMPT = (
    ...
    "das suas conversas com a Nakamura desde o último resumo.\n"
    ...
    "- fatos novos importantes sobre a Nakamura ou pessoas citadas,\n"
    "- clima geral do dia.\n"
...
def collect_transcript(memory: Any, since: datetime | None) -> str:
    ...
    for event in reversed(events):  # newest first; budget favors recent turns
        ...
        speaker = "Hana" if role in {"hana", "assistant", "model"} else "Nakamura"
        if len(content) > 600:
            content = content[:600] + "..."
    ...
    def _loop(self) -> None:
        ...
        while not self._stop.is_set():
            try:
                ...
                # Tem que LOGAR: sem isto, um refactor que renomeie algo aqui
                # dentro faz o diario parar de rodar pra sempre, em silencio
                # absoluto. Um `pass` sozinho escondia exatamente isso.
                logger.exception("Ciclo de sono falhou; tenta de novo no proximo intervalo")
            ...
            try:
                ...
                logger.warning("Falha ao indexar memorias pendentes", exc_info=True)
