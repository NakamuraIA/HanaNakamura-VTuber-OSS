...
# Fonte unica dos providers de imagem que possuem runtime executavel.
IMPLEMENTED_IMAGE_PROVIDERS = ("gemini_api", "openrouter")


def normalize_image_provider(provider: Any) -> str:
    """Normalize image provider ID strings."""
    value = str(provider or "").strip().lower()
    normalized = IMAGE_PROVIDER_ALIASES.get(value, value or DEFAULT_IMAGE_PROVIDER)
    return normalized if normalized in IMPLEMENTED_IMAGE_PROVIDERS else DEFAULT_IMAGE_PROVIDER
...
def create_image_provider(
    ...
    *,
    ...
    from backend.modules.vision.image_gen import HanaImageGen
