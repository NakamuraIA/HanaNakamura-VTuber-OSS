...
from backend.modules.vision.image_provider import BaseImageProvider, ImageProviderResult
...
class OpenRouterImageProvider(BaseImageProvider):
        ...
        if memory is not None:
            try:
                ...
                if isinstance(img_config, dict):
                    configured_model = img_config.get("imageModel") or img_config.get("openrouterImageModel")
                    if not model and configured_model:
                        self.default_model = str(configured_model).strip()
    ...
    def generate_character(self, raw_payload: str | dict) -> str | None:
        ...
        from backend.modules.vision.character_library import (
    ...
    def _call_api(
        ...
        # Always try ["image", "text"] first — most modern image models support both.
        # If the API rejects it, fall back to ["image"] only. This avoids brittle
        # catalog/heuristic checks for newly released models (e.g., krea/krea-2-medium-turbo).
        ...
        try:
            ...
            if exc.code in (400, 404) and ("modalities" in detail.lower() or "unsupported" in detail.lower() or "no endpoints found" in detail.lower()):
                logger.info("[OPENROUTER IMAGE] Model rejected image+text modalities. Retrying with image-only.")
                ...
                try:
                    ...
                    return ImageProviderResult(ok=False, error=error_msg, model=self.default_model)
            elif exc.code == 400 and "reasoning" in detail.lower() and "reasoning" in payload:
