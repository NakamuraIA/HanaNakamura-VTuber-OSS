...
_STT_MODEL_CACHE: dict[str, Any] = {"loaded_at": 0.0, "models": [], "error": None}
...
def map_openrouter_model(raw: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        # Unix timestamp de quando o modelo entrou no OpenRouter. Vinha da API e
        # era descartado aqui; e o unico jeito de ordenar por "lancado agora",
        # que e como se acha modelo novo num catalogo de ~370.
        "createdAt": _number_or_none(raw.get("created")),
    }
...
def _map_openrouter_stt_model(raw: dict[str, Any]) -> dict[str, Any]:
    """Formato bem mais simples que map_openrouter_model — nao sao modelos de chat."""
    model_id = str(raw.get("id") or "").strip()
    return {"id": model_id, "label": str(raw.get("name") or model_id), "provider": "openrouter"}


def get_openrouter_stt_catalog(*, force_refresh: bool = False) -> tuple[list[dict[str, Any]], str | None]:
    """Modelos de TRANSCRICAO do OpenRouter (?output_modalities=transcription).

    Endpoint proprio, diferente do catalogo de chat/LLM — cache separado, nunca
    se mistura com get_openrouter_catalog(). Sem tabela no banco: e "endpoint
    bom" (DECISOES_CATALOGO_FONTES.md secao 1), igual o catalogo de LLM.
    """
    now = time.monotonic()
    if not force_refresh and now - float(_STT_MODEL_CACHE.get("loaded_at") or 0) < OPENROUTER_CATALOG_CACHE_SECONDS:
        return list(_STT_MODEL_CACHE.get("models") or []), _STT_MODEL_CACHE.get("error")

    try:
        request = urllib.request.Request(
            f"{OPENROUTER_MODELS_URL}?output_modalities=transcription",
            headers=openrouter_headers(include_auth=True),
            method="GET",
        )
        ...
        if not isinstance(raw_models, list):
            raise ValueError("openrouter_stt_models_missing_data")
        models = [_map_openrouter_stt_model(item) for item in raw_models if isinstance(item, dict) and item.get("id")]
        _STT_MODEL_CACHE.update({"loaded_at": now, "models": models, "error": None})
        return list(models), None
    except urllib.error.HTTPError as exc:
        detail = exc.read().decode("utf-8", errors="replace") if exc.fp else str(exc)
        error = f"openrouter_stt_models_http_{exc.code}:{detail[:500]}"
    except Exception as exc:  # noqa: BLE001
        error = f"openrouter_stt_models_error:{exc}"

    _STT_MODEL_CACHE.update({"loaded_at": now, "models": [], "error": error})
    return [], error
