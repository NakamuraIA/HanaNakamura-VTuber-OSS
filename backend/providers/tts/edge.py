...
class TTSStreamingError(TTSConfigurationError):
    """Falha no streaming indicando se algum áudio já chegou ao alto-falante."""

    def __init__(self, message: str, *, audio_started: bool) -> None:
...
def _ffmpeg_path() -> str:
    ...
    from backend.paths import MEMORY_DB as db_path
    if db_path.exists():
        try:
...
class EdgeTTSPlayer:
        ...
        *,
            ...
            try:
                async for audio_chunk in provider.stream_audio_chunks(text):
                    ...
                    try:
                        ...
                        break
            except Exception as exc:  # provider pode falhar depois do primeiro chunk
                stream_error.append(exc)
            with contextlib.suppress(Exception):
                process.stdin.close()
        ...
        if playback_error:
            raise TTSStreamingError(
                f"Falha no playback streaming Edge TTS: {playback_error[0]}",
                audio_started=audio_started.is_set(),
            )
        if stream_error:
            raise TTSStreamingError(
                f"Falha no fluxo streaming TTS: {stream_error[0]}",
                audio_started=audio_started.is_set(),
            )
        return wrote_audio and audio_started.is_set()
    ...
    async def play_stream(
        ...
        *,
        ...
        """
        return await self.play_edge_streaming(
            provider,
            text,
            volume=volume,
