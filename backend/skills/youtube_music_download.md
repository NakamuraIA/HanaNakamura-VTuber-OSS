...
Baixa música do YouTube como MP3 direto na área de trabalho da Nakamura.
...
2. Tentar baixar direto como MP3:
   ```
   python -m yt_dlp --extract-audio --audio-format mp3 -o "C:\Users\Nakamura\Desktop\%(title)s.%(ext)s" "<URL>"
   ```
3. Se falhar, baixar como webm:
   ```
   python -m yt_dlp -f bestaudio -o "C:\Users\Nakamura\Desktop\%(title)s.%(ext)s" "<URL>"
   ```
