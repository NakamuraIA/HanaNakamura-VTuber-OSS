...
- **Personalização** theme engine (`src/theme.ts`): primary/secondary colors,
  highlight gradient (start/end/angle), background tone, live **opacity** and
  **backdrop blur** on glass panels (`.hana-glass`), plus global saturation and
  brightness. Settings persist in `localStorage` (`hana_theme_v2`) and apply as
  CSS variables on `:root` so sidebar, cards and tab shells react immediately.
