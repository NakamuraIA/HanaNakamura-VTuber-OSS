...
export const McpApi = {
  ...
  updateMcpServer: async (serverId: string, updates: Partial<McpServer>): Promise<boolean> => {
    try {
      const res = await fetch(`${BACKEND_URL}/api/mcp/servers/${encodeURIComponent(serverId)}/update`, {
        method: "POST",
