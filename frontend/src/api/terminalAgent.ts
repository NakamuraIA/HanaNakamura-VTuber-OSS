import { TerminalAgentEvent, TerminalAgentEventsResponse, TerminalAgentSpeechResponse, TerminalAgentStreamMessage, TerminalAgentTextResponse, TerminalAgentTranscriptionResponse, VoiceConfig, VoiceInputDevice, VoiceRuntimeStatus } from "../models/types";
import { backendFetch, WS_URL } from "./core";
...
export const TerminalAgentApi = {
  connectTerminalAgentStream: (onMessage: (message: TerminalAgentStreamMessage) => void): WebSocket => {
    const ws = new WebSocket(`${WS_URL}/api/terminal-agent/stream`);
    ws.onmessage = (event) => {
      try {
        onMessage(JSON.parse(event.data) as TerminalAgentStreamMessage);
      } catch {
        // Um pacote inválido não deve derrubar o Terminal.
      }
    };
    return ws;
  },
