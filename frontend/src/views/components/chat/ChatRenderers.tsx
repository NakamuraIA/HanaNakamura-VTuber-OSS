...
import { Wrench, ChevronDown, BrainCircuit, Globe2, Loader2, AlertTriangle } from "lucide-react";
...
// Camuflado de propósito: quase ninguém expande isso, então vira um textinho
// discreto (sem card colorido) que só cresce quando clicado.
  ...
  return (
    <div className="mb-2">
      <button
        type="button"
        onClick={() => setExpanded((value) => !value)}
        className="flex items-center gap-1.5 text-[10px] font-mono text-[var(--text-muted)]/70 hover:text-[var(--text-muted)] transition-colors"
      >
        <BrainCircuit size={11} />
        {memoryContext.count} {memoryContext.count === 1 ? "lembrança" : "lembranças"} usada{memoryContext.count === 1 ? "" : "s"}
...
// Mostra texto pensado E chamadas de ferramenta numa timeline única, na ordem
// em que aconteceram (era 2 cards separados — "Ferramentas" ao vivo + "Agent
// Mode" — pra cada chamada; virou uma coisa só, como o resto do mercado faz).
export function ThinkingRenderer({
  ...
  if (!items?.length && !isThinking) return null;
  ...
  // Memoizado: essa varredura e O(n^2) e "items" muda a cada token durante o
  // streaming ao vivo — sem useMemo ela rodava de novo em TODO render.
  const visible = useMemo(
    () =>
      items.filter(
        (item, i) =>
          item.type !== "tool_call" ||
          !items.some((other, j) => j > i && other.type === "tool_result" && other.tool === item.tool)
      ),
    [items]
  );
  ...
  return (
    <div className="mb-3 rounded-xl border border-purple-400/20 bg-purple-500/5">
      ...
      {expanded && (
        <div className="px-3 pb-3 flex flex-col gap-1.5 max-h-[300px] overflow-y-auto scrollbar-thin">
          {visible.length === 0 && (
          ...
          {visible.map((item, i) => {
            if (item.type === "text") {
              return (
                <p key={i} className="text-[11px] leading-relaxed text-[var(--text-muted)] whitespace-pre-wrap">
                  {item.content}
                </p>
              );
            }
            const isCall = item.type === "tool_call";
            return (
              <div key={i} className="flex items-center gap-2 text-[11px] animate-fade-in">
                {isCall ? (
                  ...
                  <span className="h-2 w-2 rounded-full bg-emerald-400 shadow-[0_0_6px_rgba(52,211,153,0.6)] shrink-0" />
                )}
                <Wrench size={11} className="text-amber-300 shrink-0" />
                <span className="font-mono text-[var(--text-secondary)]">{item.tool}</span>
                <span className="text-[var(--text-muted)]">{isCall ? "executando..." : "concluido"}</span>
              </div>
            );
          })}
        </div>
...
// Card colapsável de erro técnico do provider. Quando um turno falha, a Hana fala
// uma frase amigável ("tropeço no cérebro") — mas o erro REAL (ex: HTTP 400
// "Reasoning is mandatory") ficava só no log. Aqui ele vira visível no chat,
// fechado por padrão pra não assustar, aberto num clique pra debugar.
export function ErrorDetailRenderer({ error }: { error: string }) {
  const [expanded, setExpanded] = useState(false);
  if (!error) return null;

  return (
    <div className="mt-2 rounded-xl border border-red-400/25 bg-red-500/5">
      <button
        type="button"
        onClick={() => setExpanded((v) => !v)}
        className="flex w-full items-center justify-between gap-3 px-3 py-2 text-left"
      ...
      {expanded && (
        <div className="px-3 pb-3">
          <code className="block whitespace-pre-wrap break-all rounded-lg bg-black/40 p-2 font-mono text-[11px] leading-relaxed text-red-200/90 max-h-[200px] overflow-y-auto scrollbar-thin">
            {error}
          </code>
        </div>
      )}
