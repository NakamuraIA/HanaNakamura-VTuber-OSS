...
export interface CatalogPickerOption {
  ...
  /** Unix timestamp de entrada no catalogo, quando o provider informa. */
  createdAt?: number;
...
interface CatalogPickerProps {
  ...
  /** Popup grande centralizado (com backdrop) em vez de dropdown ancorado. */
  modal?: boolean;
}

type SortId = "name" | "recent" | "price_asc" | "price_desc" | "context" | "performance";

const SORT_LABELS: Record<SortId, string> = {
  name: "A–Z",
  recent: "Novidades",
  price_asc: "Mais barato",
  price_desc: "Mais caro",
  context: "Maior contexto",
  performance: "Mais rápido",
};

/** Empresa dona do modelo, derivada do prefixo do id ("google/gemini..." -> google). */
function companyOf(option: CatalogPickerOption): string {
...
export function CatalogPicker({
  ...
  // "Novidades" so faz sentido quando o provider informa a data de entrada
  // (hoje so o OpenRouter). Nos outros o botao ordenaria por nada.
  const hasCreatedAt = useMemo(() => options.some((option) => typeof option.createdAt === "number"), [options]);
  const hasPerformance = useMemo(() => options.some((option) => typeof option.performanceScore === "number"), [options]);
  ...
  useEffect(() => {
    ...
    const updatePosition = () => {
      if (modal) {
        // Popup grande centralizado: ocupa quase a tela toda, independente do trigger.
        const width = Math.min(920, window.innerWidth - 32);
        setPanelStyle({ left: (window.innerWidth - width) / 2, top: Math.max(16, window.innerHeight * 0.06), width });
        return;
      }
  ...
  // Empresas presentes no catálogo (prefixo do id), ordenadas por quantidade.
  // Só vira filtro quando há variedade real (ex: OpenRouter com google/, openai/...).
  const companies = useMemo(() => {
    const counts = new Map<string, number>();
    for (const option of options) {
      const company = companyOf(option);
  ...
  const filteredOptions = useMemo(() => {
    ...
    const matches = options.filter((option) => {
      ...
      if (activeCompany && companyOf(option) !== activeCompany) return false;
    ...
    const priceOf = (option: CatalogPickerOption) =>
      option.free ? 0 : (typeof option.priceScore === "number" && Number.isFinite(option.priceScore) ? option.priceScore : Number.POSITIVE_INFINITY);
    ...
    return matches.sort((left, right) => {
      ...
      // Ordenação explícita do usuário vence a implícita do filtro ativo.
      // "recent": sem data conhecida vai pro fim, senão modelo antigo do catálogo
      // legado (createdAt undefined) empataria com lançamento de ontem.
      if (sortBy === "recent") return (right.createdAt ?? 0) - (left.createdAt ?? 0);
      // Entre endpoints do MESMO modelo o preco e o contexto costumam ser
      // identicos; o que muda de verdade e velocidade. Sem esta ordenacao a
      // lista virava 20 linhas iguais e a escolha era no chute.
      if (sortBy === "performance") return (right.performanceScore ?? 0) - (left.performanceScore ?? 0);
      if (sortBy === "price_asc") return priceOf(left) - priceOf(right);
      if (sortBy === "price_desc") return priceOf(right) - priceOf(left);
      if (sortBy === "context") return (right.contextTokens ?? 0) - (left.contextTokens ?? 0);
  ...
  }, [activeCompany, activeFilter, deferredQuery, favorites, options, sortBy]);
  ...
  const panel = open && typeof document !== "undefined"
    ? createPortal(
      <>
      {modal ? <div className="fixed inset-0 z-[119] bg-black/60 backdrop-blur-sm" /> : null}
      <div
        ref={panelRef}
        id={`${id}-panel`}
        style={panelStyle}
      ...
      >
        <div className="border-b border-zinc-800 p-3">
          ...
          {/* Filtro por empresa (prefixo do id) + ordenação — só quando faz sentido */}
          {companies.length >= 2 ? (
            <div className="mt-2 flex flex-wrap gap-1.5">
              <button
                type="button"
                onClick={() => setActiveCompany("")}
                className={`inline-flex h-6 items-center rounded-md border px-2 font-mono text-[9px] font-bold uppercase transition-colors ${
                  activeCompany === ""
                    ? `${style.selected} ${style.text}`
                    : "border-zinc-800 bg-zinc-950/60 text-zinc-500 hover:border-zinc-600 hover:text-zinc-200"
                }`}
              ...
              {companies.map((company) => (
                <button
                  key={company}
                  type="button"
                  onClick={() => setActiveCompany((current) => (current === company ? "" : company))}
                  className={`inline-flex h-6 items-center rounded-md border px-2 font-mono text-[9px] font-bold uppercase transition-colors ${
          ...
          {showAdvancedFilters ? (
            <div className="mt-2 flex flex-wrap items-center gap-1.5">
              <span className="font-mono text-[9px] uppercase text-zinc-600">Ordenar:</span>
              {(Object.keys(SORT_LABELS) as SortId[])
                .filter((sort) => (sort !== "recent" || hasCreatedAt) && (sort !== "performance" || hasPerformance))
                .map((sort) => (
                <button
                  key={sort}
                  type="button"
        ...
        <div className={`${modal ? "max-h-[62vh]" : compact ? "max-h-[280px]" : "max-h-[360px]"} overflow-y-auto p-2 custom-scrollbar`}>
