...
export function OpenRouterEndpointPicker({ model, value, onChange, compact = false }: OpenRouterEndpointPickerProps) {
  ...
  const options = useMemo<CatalogPickerOption[]>(() => [
    ...
    ...endpoints.map((endpoint) => ({
      ...
      badges: [
        // Status so aparece quando NAO esta online: "unknown" repetido em 20
        // linhas era ruido puro e escondia o que diferencia um endpoint do outro.
        ...(endpoint.status && endpoint.status !== "online"
          ? [{ label: endpoint.status, tone: "amber" as const }]
          : []),
        ...(typeof endpoint.throughputLast30m === "number" && endpoint.throughputLast30m > 0
          ? [{ label: `${Math.round(endpoint.throughputLast30m)} tok/s`, tone: "cyan" as const }]
          : []),
        ...(endpoint.quantization ? [{ label: endpoint.quantization, tone: "neutral" as const }] : []),
