...
export function TabLogs() {
  ...
  return (
    <div className="w-full h-full bg-[var(--bg-sidebar)] hana-glass overflow-hidden shadow-2xl relative flex flex-col transition-all duration-500">
        ...
        {filteredLogs.length === 0 ? (
          ...
          <div className="flex flex-col">
            {filteredLogs.map((log, i) => (
              // Chave composta (nao so o indice): logs chegam a cada 1.5s e o filtro de
              // nivel muda quais linhas aparecem, entao o indice sozinho reciclava a
              // linha errada do React quando a lista encolhia/crescia.
              <div key={`${log.timestamp}-${log.logger}-${i}`} className={`flex items-start gap-4 py-1.5 px-3 hover:bg-white/5 transition-colors ${getRowBg(log.level, i)}`}>
